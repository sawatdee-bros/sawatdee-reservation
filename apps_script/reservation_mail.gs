 ...
 *    （URLは変わらない・reservation_settings の email_endpoint の再設定不要）
 * ❌ 誤：「新しいデプロイ」を選ぶ
 *    （新URLが発行され、本店 reservation_settings の email_endpoint は古いコード指したまま）
 *
 * 過去事例：2026-04-23 売上バックアップGASでこの誤手順により1週間売上欠損。
 *
 * 【MVP段階の制約】
 * テンプレートはGAS内ハードコード。
 * SaaS化時に Firebase reservation_settings/email_template への移行検討。
 * 差し込み変数（店名・電話・予約日時等）と本文の境界を明確に保つこと（移行容易化のため）。
 *
 * 【店名の取得元】
 * 店名は POS側 /store_config/store_name から取得（重複データ回避）。
 * 電話・住所は予約側 /tenants/{tid}/reservation_settings/store_info から取得。
 * 将来SaaS化時に store_info を POS store_config に統合する想定。
 *
 * 【プロジェクト管理】
 * このスクリプトはスタンドアロン GAS プロジェクト「サワディ兄弟 予約メール送信」として
 * 単独運用。POS の firebase_export.gs / 売上バックアップ用GAS とは別プロジェクト。
 */

// Firebase Realtime Database ベースURL（POSと共通プロジェクト・asia-southeast1）
 ...
 * {
 *   tenant_id:      string,    // 必須・テナントID（本店時 "sawatdee-bros"）
...
/**
 * 件名テンプレ。差し込み変数を使う形で本文と分離（将来Firebase化時に対応容易）。
 */
...
function buildBody(v) {
  var lines = [
    ...
    'ご人数:     ' + v.party_size + '名様',
    '予約番号:   ' + v.reservation_no,
    '━━━━━━━━━━━━━━━━━━━━',
    '',
    '【ご来店時のお願い】',
    '・ご予約時刻を15分過ぎてもご連絡いただけない場合、',
    '  キャンセル扱いとなる場合がございます。',
...
/**
 * Apps Script Web App は HTTPステータスコードを直接返せないため、
...
/**
 * 手動テスト用。GASエディタで実行ボタンから叩いて、Firebase 接続と送信が動くか確認する。
